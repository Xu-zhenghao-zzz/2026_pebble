"""Utrn TCGA clinical validation (v2 Phase 5).

Mirrors v1's BLNK validation but for Utrn, our novel causal hub.

Three analyses:
  (1) LUAD expression correlation with stromal/immune markers
  (2) Pan-cancer Utrn-CD8A correlation (117 TCGA cancer types)
  (3) LUAD survival: Utrn-high vs Utrn-low

cBioPortal API. Outputs:
  - processed/utrn_tcga_validation.json
  - figures/F7_utrn_tcga.png
"""
...
# Utrn = entrez 22290; reference v1 markers
GENES = {
    "UTRN": 22290,
...
def main():
    print("[tcga] fetching expression for Utrn + markers in LUAD...", flush=True)
    expr = {}
    ...
    if "UTRN" not in expr:
        print("[tcga] FAILED to fetch Utrn; abort")
        return
    ...
    for group, markers in marker_groups.items():
        for m in markers:
            if m in df.columns and "UTRN" in df.columns:
                rho, p = spearmanr(df["UTRN"], df[m])
                results["correlations"][f"UTRN_vs_{m}"] = {
                    "group": group, "rho": float(rho), "p": float(p), "n": len(df),
                }

    # Utrn high vs low (top/bottom quartile) marker comparison
    if "UTRN" in df.columns:
        q25, q75 = df["UTRN"].quantile([0.25, 0.75])
        high = df[df["UTRN"] >= q75]
        low = df[df["UTRN"] <= q25]
        results["high_vs_low"] = {}
    ...
    # save
    with open(PROCESSED / "utrn_tcga_validation.json", "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nwrote: processed/utrn_tcga_validation.json")
    ...
    ax.set_xticks(x)
    ax.set_xticklabels([n.replace("UTRN_vs_", "") for n in names],
                       rotation=45, ha="right")
    ax.axhline(0, color="gray", lw=0.5)
    ax.set_ylabel("Spearman ρ with UTRN")
    ax.set_title(f"(a) Utrn ↔ marker correlations (TCGA LUAD, n={len(df)})")
    # legend
    ...
    if hv:
        ...
        width = 0.35
        x = np.arange(len(ms))
        ax.bar(x - width/2, low_means, width, label="Utrn-low (Q1)",
               color="#9575cd", alpha=0.7)
        ax.bar(x + width/2, high_means, width, label="Utrn-high (Q4)",
               color="#2e7d32", alpha=0.7)
        ax.set_xticks(x)
        ax.set_xticklabels(ms, rotation=30, ha="right")
        ax.set_ylabel("Mean expression (TPM)")
        ax.set_title("(b) Utrn-high vs low marker levels")
        ax.legend()
        ax.grid(alpha=0.3, axis="y")

    fig.suptitle("Utrn TCGA LUAD clinical validation (v2)",
                 fontsize=13, fontweight="bold")
    fig.tight_layout()
    out = FIGS / "F7_utrn_tcga.png"
    fig.savefig(out, dpi=150, bbox_inches="tight")
    print(f"wrote: {out}")

    # print summary
    print("\n=== Utrn TCGA correlations (LUAD, n={}) ===".format(len(df)))
