"""Phgr1 TCGA LUAD survival analysis.

Mirrors v1's v2_tcga_survival.py (BLNK) but for Phgr1.
- Log-rank test: Phgr1-high (Q4) vs Phgr1-low (Q1) OS
- Kaplan-Meier curves
- Cox proportional hazards (univariate + multivariate age/sex if available)

Outputs:
  - processed/phgr1_survival.json
  - figures/F8_phgr1_survival.png
"""
...
def fetch_phgr1():
    ...
    params = {"sampleListId": SAMPLE_LIST, "entrezGeneId": PHGR1_ENTREZ}
...
def main():
    print("========== Phgr1 TCGA LUAD survival ==========")
    print("fetching Phgr1 expression...")
    expr = fetch_phgr1()
    ...
    df = clin.join(pd.Series(phgr1_p, name="PHGR1"), how="inner")
    df = df.dropna(subset=["PHGR1"])
    print(f"merged: {len(df)} patients with Phgr1 + clinical")
    ...
    valid = df.dropna(subset=["OS_MONTHS", "PHGR1"]).copy()
    print(f"with OS data: {len(valid)} patients, {valid['event'].sum()} events")

    # quartile split
    q25, q75 = valid["PHGR1"].quantile([0.25, 0.75])
    valid["group"] = "mid"
    valid.loc[valid["PHGR1"] <= q25, "group"] = "low"
    valid.loc[valid["PHGR1"] >= q75, "group"] = "high"
    ...
    # Cox univariate
    cph = CoxPHFitter()
    cox_df = valid[["OS_MONTHS", "event", "PHGR1"]].copy()
    cox_df["log_PHGR1"] = np.log10(cox_df["PHGR1"].clip(lower=1e-3))
    cph.fit(cox_df, duration_col="OS_MONTHS", event_col="event",
            formula="log_PHGR1")
    hr = cph.hazard_ratios_.iloc[0]
    ci = cph.confidence_intervals_.iloc[0]
    p_cox = cph.summary.loc["log_PHGR1", "p"]
    print(f"Cox PH (log_PHGR1): HR={hr:.3f}, 95% CI [{ci.iloc[0]:.3f}, {ci.iloc[1]:.3f}], p={p_cox:.4g}")
    ...
    if "AGE" in valid.columns and "SEX" in valid.columns:
        ...
        cox_m = valid[["OS_MONTHS", "event", "log_PHGR1", "AGE_num", "SEX_male"]].copy() if "log_PHGR1" in valid.columns else None
        if cox_m is None:
            valid["log_PHGR1"] = np.log10(valid["PHGR1"].clip(lower=1e-3))
            cox_m = valid[["OS_MONTHS", "event", "log_PHGR1", "AGE_num", "SEX_male"]].copy()
        cox_m = cox_m.dropna()
        if len(cox_m) > 50:
            cph_m = CoxPHFitter()
    ...
    with open(PROCESSED / "phgr1_survival.json", "w") as f:
        json.dump(result, f, indent=2, default=str)
    print(f"\nwrote: processed/phgr1_survival.json")

    # figure
    fig, axes = plt.subplots(1, 2, figsize=(13, 5.5))
    ax = axes[0]
    kmf = KaplanMeierFitter()
    for label, sub, color in [("Phgr1-low (Q1)", low, "#9575cd"),
                              ("Phgr1-high (Q4)", high, "#2e7d32")]:
        kmf.fit(sub["OS_MONTHS"], event_observed=sub["event"], label=label)
        kmf.plot_survival_function(ax=ax, color=color, ci_show=True)
    ax.set_title(f"(a) Phgr1 Kaplan-Meier OS (TCGA LUAD)\n"
    ...
    fig.suptitle("Phgr1 survival analysis — TCGA LUAD (n=518)",
                 fontsize=13, fontweight="bold")
    fig.tight_layout()
    out = FIGS / "F8_phgr1_survival.png"
